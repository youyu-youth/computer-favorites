...
<template>
  <transition-group
    ...
    <div v-for="server in servers" :key="server.title"
         :class="[
             'bg-white dark:bg-[#1a2126] border border-gray-200 dark:border-darkBorder rounded-xl overflow-hidden hover:border-gray-300 dark:hover:border-gray-600 transition-all hover:shadow-md dark:shadow-none',
        ...
        <div :class="[viewMode === 'list' ? 'flex-grow p-4' : '']">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div class="flex items-center gap-2">
                            <a href="#" class="text-brandBlue font-medium hover:underline">{{ server.title }}</a>
                            <span v-if="server.isOfficial" class="px-1.5 py-0.5 rounded text-[10px] font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-brandBlue border border-blue-200 dark:border-blue-800/50">official</span>
                        </div>
            ...
            <div class="flex flex-wrap items-center gap-4 mb-3">
                <div v-for="tag in server.tags" :key="tag.name" class="flex items-center gap-1.5 text-xs text-gray-600 dark:text-gray-400">
                    <div class="w-4 h-4 rounded-full border border-gray-300 dark:border-darkBorder flex items-center justify-center bg-white dark:bg-darkBg">
                        <span class="text-[8px] font-bold" :class="tag.status === 'good' ? 'text-brandGreen' : 'text-gray-400'">A</span>
                    </div>
                    <span>{{ tag.name }}</span>
                </div>
            </div>
