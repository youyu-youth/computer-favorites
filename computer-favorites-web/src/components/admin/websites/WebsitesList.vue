  ...
  >
    ...
    <div
      ...
      :class="[
        ...
        viewMode === 'list'
          ? 'flex flex-col sm:flex-row items-center p-3 gap-4 sm:pr-4 overflow-hidden hover:border-gray-300 dark:hover:border-gray-600'
          : 'flex flex-col p-5 overflow-hidden hover:border-gray-300 dark:hover:border-gray-600',
      ]"
    >
      <!-- ======================= -->
      <!-- GRID VIEW CONTENT -->
      ...
      <template v-if="viewMode === 'grid'">
        ...
        >
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                <div class="flex items-center gap-2">
                  <span class="text-brand-blue font-medium hover:underline">{{
                    server.title
                  }}</span>
                  <span
                    v-if="server.isOfficial"
                    class="px-1.5 py-0.5 rounded text-[10px] font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-brand-blue border border-blue-200 dark:border-blue-800/50"
                    >official</span
                  >
                </div>
          ...
          <div class="flex flex-wrap items-center gap-4 mb-3">
            ...
            >
              <div
                class="w-4 h-4 rounded-full border border-gray-300 dark:border-dark-border flex items-center justify-center bg-white dark:bg-dark-bg"
              >
                <span
                  class="text-[8px] font-bold"
                  :class="tag.status === 'good' ? 'text-brand-green' : 'text-gray-400'"
                  >A</span
                >
              </div>
              <span>{{ tag.name }}</span>
        ...
        <div class="mt-auto flex items-center justify-between gap-3 pt-2">
          <div
            ...
            :class="resolveStatusTextClass(server)"
          >
            <span
              aria-hidden="true"
              class="h-2 w-2 rounded-full"
              :class="resolveStatusDotClass(server)"
            ></span>
          ...
          >
            ...
            />
            <span>{{
              isStatusUpdating(server.id) ? '处理中...' : resolveStatusActionLabel(server)
            }}</span>
          </button>
        </div>
      </template>
      ...
      <template v-else>
        ...
        <div class="hidden sm:flex pl-1">
          ...
          >
            <i
              :class="
                isSelected(server.id) ? 'fas fa-check-square text-lg' : 'far fa-square text-lg'
              "
            ></i>
          </button>
        </div>
        ...
        <div class="flex-grow min-w-0 pr-4 cursor-pointer" @click="handleViewDetail(server)">
          <div class="flex items-center gap-3 mb-0.5">
            <span class="text-brand-blue font-medium hover:underline truncate">{{
              server.title
            }}</span>
            <span
              v-if="server.isOfficial"
              class="px-1.5 py-0.5 rounded text-[10px] font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-brand-blue border border-blue-200 dark:border-blue-800/50 flex-shrink-0"
              >official</span
            >
            <span class="text-gray-400 dark:text-gray-500 text-xs truncate max-w-[120px]">{{
              server.author
            }}</span>
          </div>
            ...
            <div class="hidden md:flex items-center gap-3 flex-shrink-0">
              ...
              >
                <span
                  ...
                  :class="tag.status === 'good' ? 'bg-emerald-400' : 'bg-gray-400'"
                ></span>
                <span>{{ tag.name }}</span>
              </div>
            </div>
        ...
        <div class="flex items-center justify-end gap-5 flex-shrink-0">
          ...
          <!-- Status indicator -->
          <div
            class="inline-flex items-center gap-1.5 text-[11px] font-medium whitespace-nowrap min-w-[4rem]"
            :class="resolveStatusTextClass(server)"
          >
            <span
              aria-hidden="true"
