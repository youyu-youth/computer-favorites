...
const publishTimeValue = computed<Date | null>({
  get: () => (props.form.publishTime ? new Date(props.form.publishTime) : null),
  set: (value: Date | null) => {
    if (value) {
      ...
      updateField('publishTime', `${y}-${m}-${d} ${h}:${min}:${s}`)
    } else {
      updateField('publishTime', '')
    }
  },
})
</script>
