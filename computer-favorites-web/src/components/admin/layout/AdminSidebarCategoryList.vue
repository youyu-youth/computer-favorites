...
const resolveCategoryIcon = (name: string, isActive: boolean) => {
  ...
  if (n.includes('typescript') || n.includes('javascript')) return { icon: 'fab fa-js', color: colorClass }
  if (n.includes('developer tools') || n.includes('tools') || n.includes('工具')) return { icon: 'fas fa-hammer', color: colorClass }
  if (n.includes('claimed')) return { icon: 'fas fa-check-circle', color: colorClass }
  if (n.includes('automation') || n.includes('自动化')) return { icon: 'fas fa-dharmachakra', color: colorClass }
  if (n.includes('search') || n.includes('搜索')) return { icon: 'fas fa-search', color: colorClass }
  if (n.includes('database') || n.includes('数据库')) return { icon: 'fas fa-database', color: colorClass }
  if (n.includes('rag') || n.includes('knowledge') || n.includes('documentation') || n.includes('知识')) return { icon: 'fas fa-book', color: colorClass }
  if (n.includes('agent') || n.includes('orchestration') || n.includes('智能体')) return { icon: 'fas fa-robot', color: colorClass }
  if (n.includes('code execution') || n.includes('代码')) return { icon: 'fas fa-code', color: colorClass }
