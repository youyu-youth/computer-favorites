...
const sites: SiteItem[] = [
  {
    ...
    detailDesc: '全球最大的开源代码托管平台，拥有超过1亿开发者。在这里，你可以托管代码、审查变更、管理项目，并与其他开发者构建软件。从个人项目到企业级应用，GitHub都是协作的核心。',
  ...
  },
  {
    ...
    detailDesc: '全球最大的开发者问答社区。无论你遇到什么编程难题，几乎都能在这里找到答案。通过提问和回答，你不仅能解决问题，还能建立自己的声誉，帮助他人成长的同时自我提升。',
  ...
  },
  {
    ...
    name: 'Vue',
    desc: '渐进式框架，优雅与效率的平衡。',
    detailDesc: '渐进式JavaScript框架，易学易用，性能出色，适用场景丰富。Vue的核心库只关注视图层，不仅易于上手，还便于与第三方库或既有项目整合。配合现代化的工具链，Vue能够驱动复杂的单页应用。',
  ...
  },
  {
    ...
    detailDesc: '开发者技能树与路线图。不仅提供技术的学习路径，还整合了最佳实践、工具推荐和资源链接。通过结构化的知识体系，帮助开发者系统性地掌握新技术，从入门到精通，不再迷失在碎片化的信息中。',
    href: '#',
    tag: 'Toolkit',
  },
]
...
const cardBgClass = (id: SiteKey) => {
  if (id === 'github') return 'bg-rose-50/85 hover:bg-rose-100/90 dark:bg-black/20 dark:hover:bg-white/10'
  if (id === 'stackoverflow') return 'bg-red-50/85 hover:bg-red-100/90 dark:bg-black/20 dark:hover:bg-white/10'
  if (id === 'vue') return 'bg-pink-50/85 hover:bg-pink-100/90 dark:bg-black/20 dark:hover:bg-white/10'
  ...
  <div class="relative min-h-dvh w-full overflow-hidden bg-slate-50 dark:bg-black">
    <div class="pointer-events-none absolute inset-0">
      ...
      <svg class="absolute inset-0 h-full w-full opacity-40 dark:opacity-25" aria-hidden="true">
        ...
        <rect width="100%" height="100%" fill="url(#grid-24)" class="text-slate-200 dark:text-white/10" />
      </svg>
    </div>
    ...
    <div class="relative mx-auto flex min-h-dvh max-w-6xl items-center px-10 py-10">
      <div class="w-full">
        ...
        >
          <div class="flex items-center gap-2 border-b border-slate-200/70 bg-white/60 px-4 py-3 dark:border-white/10 dark:bg-white/5">
            ...
            <div class="ml-3 truncate text-xs text-slate-500 dark:text-slate-400">preview://favorites</div>
          </div>
          ...
          <div class="grid grid-cols-1 gap-6 p-6 xl:grid-cols-[minmax(0,1fr)_320px]">
              ...
              <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                >
                  <div class="flex items-start justify-between gap-3">
                    ...
                    >
                      <svg v-if="s.id === 'github'" class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
                        <path
                      ...
                      <svg v-else-if="s.id === 'vue'" class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
                        <path
                          d="M24,1.61H14.06L12,5.16,9.94,1.61H0L12,22.39ZM12,14.08,5.16,2.23H9.59L12,6.41l2.41-4.18h4.43Z"
                        />
                      </svg>
                      <svg v-else-if="s.id === 'skills'" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                ...
                >
                  <div class="flex items-center justify-between">
                    <div class="text-sm font-semibold text-slate-900 dark:text-white">Code Editor</div>
                    <div class="text-xs text-slate-500 dark:text-slate-400">TypeScript</div>
                  </div>
                  <div class="mt-3 relative">
                    <div class="overflow-hidden rounded-xl border border-rose-200/70 bg-[#111217] shadow-[0_10px_40px_-24px_rgba(15,23,42,0.8)] dark:border-white/10">
                      <div class="flex items-center justify-between border-b border-white/10 bg-[#1a1b22] px-4 py-2">
                        <div class="flex items-center gap-2">
                        ...
                        <div class="text-[10px] tracking-[0.2em] text-slate-400">TREASURE_VAULT.JS — EDITOR</div>
                      </div>
                      <div class="grid grid-cols-[32px_minmax(0,1fr)] gap-3 px-4 py-3 font-mono text-[14px] leading-7">
                        ...
                        <div class="text-slate-200">
                          <div><span class="text-violet-300">const</span> <span class="text-cyan-300">devTreasure</span> <span class="text-amber-300">=</span> <span class="text-amber-300">{</span></div>
                          <div>&nbsp;&nbsp;<span class="text-slate-300">status</span><span class="text-amber-300">:</span> <span class="text-emerald-300">'Ready'</span><span class="text-amber-300">,</span></div>
                          <div>&nbsp;&nbsp;<span class="text-slate-300">passion</span><span class="text-amber-300">:</span> <span class="text-lime-300">Infinity</span><span class="text-amber-300">,</span></div>
                          <div>&nbsp;&nbsp;<span class="text-slate-300">action</span><span class="text-amber-300">:</span> <span class="text-amber-300">()</span> <span class="text-violet-300">=&gt;</span> <span class="text-yellow-300">explore</span><span class="text-amber-300">()</span></div>
            ...
            >
              <div class="text-xs font-medium tracking-wide text-slate-500 dark:text-slate-400">PREVIEW</div>
              <div class="mt-2 text-base font-semibold text-slate-900 dark:text-white">{{ active.name }}</div>
              <div class="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-400">
                {{ active.desc }}
              </div>
              <div class="mt-4 overflow-hidden rounded-lg border border-rose-200/80 bg-rose-50/70 dark:border-white/10 dark:bg-black/30">
                ...
                <div class="flex items-center justify-between border-t border-slate-200 px-3 py-2 dark:border-white/10">
