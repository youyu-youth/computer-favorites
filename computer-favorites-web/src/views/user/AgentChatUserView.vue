<template>
  <div
    class="cf-chat-shell relative flex h-full flex-col overflow-hidden bg-stone-50 text-stone-900 dark:bg-[#000000] dark:text-stone-200"
  >
    <header
      class="cf-chat-topbar z-10 flex shrink-0 items-center gap-3 border-b border-stone-200/80 bg-white/95 px-4 py-3 md:px-6 dark:border-white/10 dark:bg-[#050505]"
    >
      <button
        type="button"
      ...
      <div class="flex min-w-0 flex-1 items-center gap-2">
        ...
        />
        <h1
          class="m-0 flex min-w-0 items-center gap-2 truncate text-[15px] font-semibold text-stone-900 dark:text-stone-100"
        >
          <span class="truncate">{{ currentTitle }}</span>
        </h1>
        <span
          class="hidden items-center text-[10px] font-medium tracking-[0.08em] text-stone-500 sm:inline-flex dark:text-stone-600"
        >
          {{ connectionLabel }}
        </span>
      </div>
      ...
      <div v-else class="mx-auto max-w-4xl space-y-8">
        ...
        >
          ...
          <div class="min-w-0 flex-1">
            <p class="m-0 text-[13px] leading-relaxed text-rose-700 dark:text-rose-300">
              {{ store.errorMessage || '连接出错，请重试' }}
            </p>
            <button
              type="button"
              class="mt-1 inline-flex cursor-pointer items-center gap-1 text-[11px] font-medium text-rose-600 transition-colors hover:text-rose-700 dark:text-rose-400 dark:hover:text-rose-300"
              @click="clearError"
            >
              知道了
            </button>
        ...
        >
          ...
          <p class="m-0 text-[13px] text-stone-700 dark:text-stone-300">
            今日对话配额已用完，请明天再来
          </p>
        </div>
      </div>
    ...
    <div class="shrink-0 bg-stone-50 dark:bg-[#000000]">
...
<style scoped>
.cf-chat-topbar {
  box-shadow: inset 0 -1px 0 0 rgb(0 0 0 / 0.04);
}
