...
<template>
  <div
    class="admin-profile-page min-h-screen bg-[#f3f6f9] text-slate-700 dark:bg-[#0e1317] dark:text-slate-300 font-mono transition-colors duration-200"
  >
    <main class="flex-grow max-w-6xl w-full mx-auto px-3 sm:px-6 lg:px-8 py-5 sm:py-8">
      ...
      <div class="mb-6 sm:mb-8 border-b border-[#cbd5e1] dark:border-[#273138] pb-3 sm:pb-4">
        <div class="flex flex-wrap items-center justify-between gap-3">
          <div
            class="text-sm text-slate-500 dark:text-gray-400 flex flex-wrap items-center gap-2 break-all min-w-0"
          >
            <span class="text-[#e95322]">{{ adminInfo.username || 'admin_root' }}@sys</span>:<span
              class="text-[#3b82f6]"
              >~</span
            >$ cat /etc/admin/profile.conf
      ...
      <div
        v-if="isLoading"
        class="border border-[#cbd5e1] dark:border-[#273138] bg-white dark:bg-[#151b21] p-6 text-sm text-slate-600 dark:text-gray-300"
      >
      ...
      <div
        v-else-if="loadError"
        class="border border-[#cbd5e1] dark:border-[#273138] bg-white dark:bg-[#151b21] p-6"
      >
      ...
      <div v-else class="grid grid-cols-1 xl:grid-cols-[320px_minmax(0,1fr)] gap-5 lg:gap-8">
        ...
        <aside class="w-full space-y-5 sm:space-y-6">
          <!-- Avatar & Identity -->
          <div
            class="border border-[#cbd5e1] dark:border-[#273138] bg-white dark:bg-[#151b21] p-5 sm:p-6 relative group hover:border-[#e95322] transition-colors"
          >
            ...
            <div class="flex flex-col items-center">
              <div
                class="w-24 h-24 mb-4 border-2 border-[#cbd5e1] dark:border-[#273138] group-hover:border-[#e95322] overflow-hidden bg-[#f8fafc] dark:bg-[#0e1317] flex items-center justify-center p-1 transition-colors"
              >
                <img
                ...
                <i
                  v-else
                  class="pi pi-user text-4xl text-slate-500 dark:text-gray-500 group-hover:text-[#e95322] transition-colors"
                ></i>
              </div>

              <h2
                class="text-xl text-slate-900 dark:text-white font-bold mb-1 uppercase tracking-wider break-all text-center"
              >
                {{ adminInfo.username || 'UNKNOWN' }}
              </h2>
              <p class="text-sm text-slate-500 dark:text-gray-500 mb-4 text-center">
                "{{ adminInfo.nickname || 'NO_NICKNAME' }}"
              </p>

              <div class="flex gap-2 w-full flex-wrap sm:flex-nowrap">
                <div
                  class="flex-1 text-center border border-[#3b82f6]/30 bg-[#3b82f6]/10 text-[#3b82f6] py-1 text-xs uppercase tracking-wider"
                >
                ...
                <div
                  :class="
                    adminInfo.status === 1
                      ? 'border-[#e95322]/30 bg-[#feece6] dark:bg-[#3e1a10] text-[#e95322]'
                      : 'border-slate-400 dark:border-gray-600 bg-slate-200 dark:bg-gray-800 text-slate-500 dark:text-gray-400'
                  "
          ...
          <!-- Sys Info Log -->
          <div
            class="border border-[#cbd5e1] dark:border-[#273138] bg-white dark:bg-[#0e1317] p-4 sm:p-5"
          >
            <h3
              class="text-slate-500 dark:text-gray-500 text-xs uppercase tracking-widest mb-3 border-b border-[#cbd5e1] dark:border-[#273138] pb-1"
            >
              >> SYS_INFO_LOG
            </h3>
        ...
        <div class="min-w-0">
          <div
            class="border border-[#cbd5e1] dark:border-[#273138] bg-white dark:bg-[#0e1317] relative"
          >
            <!-- Header -->
            <div
              class="bg-[#eef2f6] dark:bg-[#151b21] border-b border-[#cbd5e1] dark:border-[#273138] px-4 py-3 flex items-center justify-between gap-3"
            >
              <h3
                class="text-slate-600 dark:text-gray-400 text-sm uppercase tracking-widest min-w-0 truncate"
              >
            ...
            <form @submit.prevent="saveProfile" class="p-4 sm:p-6 space-y-6">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6">
                ...
                <div>
                  <label
                    class="block text-xs text-slate-500 dark:text-gray-500 uppercase tracking-widest mb-2"
                    >USERNAME_ID</label
                  >
                  <div
                    class="relative flex items-center bg-[#f8fafc] dark:bg-[#151b21] border border-[#cbd5e1] dark:border-[#273138] opacity-80"
                  >
                    ...
                    <input
                      ...
                      disabled
                      class="w-full bg-transparent text-slate-600 dark:text-gray-400 p-2 outline-none font-mono text-sm cursor-not-allowed"
                    />
                  </div>
                  <p class="mt-1 text-[10px] text-slate-500 dark:text-gray-600">
                ...
                <div>
                  <label
                    ...
                    :class="isEditing ? 'text-[#e95322]' : 'text-slate-500 dark:text-gray-500'"
                    >NICKNAME</label
                  >
                  <div
                    class="relative flex items-center border transition-colors bg-white dark:bg-black/80"
                    :class="
                      isEditing ? 'border-[#e95322]' : 'border-[#cbd5e1] dark:border-[#273138]'
                    "
                  >
                    <span
                      class="pl-3 font-bold"
                      :class="isEditing ? 'text-[#e95322]' : 'text-slate-500 dark:text-gray-600'"
                      >></span
                    >
                    <input
                      type="text"
                      ...
                      class="w-full bg-transparent text-slate-800 dark:text-gray-200 p-2 outline-none font-mono text-sm disabled:text-slate-500 dark:disabled:text-gray-500"
                    />
                  </div>
                </div>
                ...
                <div>
                  <label
                    class="block text-xs uppercase tracking-widest mb-2"
                    :class="isEditing ? 'text-[#e95322]' : 'text-slate-500 dark:text-gray-500'"
                    >EMAIL_ADDRESS <span v-if="isEditing" class="text-red-500">*</span></label
                  >
                  <div
                    class="relative flex items-center border transition-colors bg-white dark:bg-black/80"
                    :class="
              ...
              <!-- Password Alert -->
              <div
                class="mt-8 border border-[#cbd5e1] dark:border-[#273138] border-dashed p-4 bg-yellow-500/10 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"
              >
                <div class="min-w-0">
                  <h4 class="text-sm text-slate-700 dark:text-gray-300 mb-1">
                    SECURITY_KEYS (PASSWORD)
                  </h4>
                  <p class="text-xs text-slate-500 dark:text-gray-600">
                    BCrypt hashed. Requires specific permissions to alter.
                  </p>
                </div>
                <button
                  type="button"
                  @click="openPasswordDialog"
                  class="w-full sm:w-auto border border-slate-400 dark:border-gray-600 text-slate-600 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white hover:border-slate-700 dark:hover:border-white px-3 py-2 sm:py-1 text-xs uppercase transition-colors cursor-pointer"
                >
                  --reset-passwd
                </button>
              </div>
              ...
              <!-- Action Buttons -->
              <div
                class="pt-6 border-t border-[#cbd5e1] dark:border-[#273138] flex flex-col-reverse sm:flex-row sm:justify-end gap-3 mt-6"
              >
                <template v-if="!isEditing">
                  <button
                    type="button"
                    @click="enableEdit"
                    class="w-full sm:w-auto bg-transparent dark:bg-[#0e1317] border border-[#e95322] text-[#e95322] px-6 py-2 text-sm font-bold hover:bg-[#e95322] hover:text-white transition-colors uppercase cursor-pointer"
                  >
                ...
                <template v-else>
                  <button
                    type="button"
                    @click="cancelEdit"
                    class="w-full sm:w-auto bg-transparent border border-slate-400 dark:border-gray-600 text-slate-600 dark:text-gray-400 px-6 py-2 text-sm font-bold hover:border-slate-700 dark:hover:border-white hover:text-slate-900 dark:hover:text-white transition-colors uppercase cursor-pointer"
                  >
                    Abort
                  </button>
                  <button
                    type="submit"
                    :disabled="isSaving"
                    class="w-full sm:w-auto bg-[#e95322] border border-[#e95322] text-white px-6 py-2 text-sm font-bold hover:opacity-90 disabled:opacity-50 transition-colors uppercase flex items-center justify-center cursor-pointer"
                  >
    ...
    >
      <template #header>
        <div class="flex items-center gap-2 min-w-0">
          ...
          <span
            class="text-sm sm:text-base font-semibold text-slate-800 dark:text-gray-100 uppercase tracking-wider truncate"
            >Modify_Admin_Password</span
          >
        </div>
      </template>
      ...
      <form id="admin-password-form" class="space-y-4" @submit.prevent="changePassword">
        <div
          v-if="passwordError"
          class="rounded border border-red-300 dark:border-red-800 bg-red-50 dark:bg-red-950/40 px-3 py-2 text-xs text-red-600 dark:text-red-300"
        >
          {{ passwordError }}
        </div>
        ...
        <div>
          <label
            class="block text-xs uppercase tracking-widest text-slate-500 dark:text-gray-500 mb-2"
        ...
        <div>
          ...
          />
          <div
            class="mt-2 flex items-center justify-between text-[11px] text-slate-500 dark:text-gray-500"
          >
