...
// 终端日志
const terminalLogs = ref<string[]>(['[系统] 正在初始化安全协议...', '[系统] 正在等待管理员凭据...'])
...
<template>
  <div
    class="min-h-screen flex flex-col md:flex-row bg-surface-page dark:bg-dark-bg font-mono transition-colors duration-300 relative overflow-hidden"
  >
    <!-- Background Grid Pattern -->
    <div
      class="absolute inset-0 pointer-events-none opacity-[0.03] dark:opacity-[0.05] z-0"
      style="
        background-image:
    ...
    <div
      class="hidden md:flex flex-1 flex-col justify-between p-10 lg:p-16 border-r border-gray-200 dark:border-dark-border bg-white/50 dark:bg-dark-card/30 backdrop-blur-sm z-10 relative"
    >
      <!-- Glow effect behind terminal text -->
      <div
        class="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-brand-orange/10 dark:bg-brand-orange/5 blur-3xl rounded-full pointer-events-none"
      ></div>

      <div>
        <div class="flex items-center gap-3 mb-6">
          <div
            class="w-10 h-10 rounded-sm bg-brand-orange flex items-center justify-center text-white shadow-[0_0_15px_rgba(233,83,34,0.4)]"
          >
            <i class="pi pi-server text-xl"></i>
          </div>
          <h1
            class="text-2xl font-bold tracking-tight text-gray-900 dark:text-gray-100 flex items-center"
          >
            CF_控制台
        ...
        <div
          class="font-mono text-xs sm:text-sm bg-gray-50 dark:bg-black/30 border border-gray-200 dark:border-dark-border p-4 rounded-sm shadow-inner h-64 overflow-hidden flex flex-col justify-end text-gray-600 dark:text-[#a0a5aa]"
        >
          <div
            ...
            :class="{
              'text-brand-orange dark:text-brand-orange font-bold': log.includes('[认证]'),
            }"
          >
            {{ log }}
          </div>
      ...
      <div
        class="text-xs text-gray-400 dark:text-gray-500 font-mono mt-8 uppercase tracking-widest flex justify-between"
      >
    ...
    <div class="flex-1 flex items-center justify-center p-6 sm:p-12 z-10">
      <div
        class="w-full max-w-md bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border rounded-sm shadow-2xl overflow-hidden group"
      >
        <!-- Top bar (mimics a window frame) -->
        <div
          class="h-8 border-b border-gray-100 dark:border-dark-border bg-gray-50 dark:bg-black/20 flex items-center px-4 gap-2"
        >
          ...
          <span class="ml-auto text-[10px] text-gray-400 font-sans tracking-wide uppercase"
            >认证模块</span
          >
        </div>
        ...
        <div class="p-8 sm:p-10">
          <div class="mb-10 text-center md:text-left">
            <h2
              class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white uppercase tracking-wider min-h-[32px]"
            >
              {{ typedTitle
              }}<span class="animate-pulse" v-if="typedTitle.length < fullTitle.length">_</span>
            </h2>
          ...
          >
            ...
            <div class="space-y-2">
              <label
                for="username"
                class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-widest"
              >
                <i class="pi pi-user mr-1 text-brand-orange"></i> 账号
              </label>
              <div class="relative flex items-center isolate">
                <span
                  class="absolute left-3 text-gray-400 dark:text-gray-500 font-bold select-none pointer-events-none z-[1]"
                  >&gt;</span
                >
                ...
                <InputText
                  ...
                  :pt="{
                    root: {
                      class:
                        '!border-gray-200 dark:!border-dark-border focus:!border-brand-orange outline-none shadow-none',
                    },
                  }"
            ...
            <div class="space-y-2">
              <div class="flex justify-between items-center">
                ...
                <a
                  href="#"
                  class="text-[10px] text-gray-500 hover:text-brand-orange transition-colors uppercase"
                  >绕过?</a
                >
              </div>
              <div class="admin-login-password-field relative flex items-center isolate">
                <span
                  class="absolute left-3 text-gray-400 dark:text-gray-500 font-bold select-none z-[1] pointer-events-none"
                  >&gt;</span
                >
                <Password
                  inputId="password"
                  ...
                  :pt="{
                    ...
                    input: {
                      class:
                        'w-full pl-10 pr-10 py-3 bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-dark-border text-gray-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-brand-orange focus:border-brand-orange transition-colors font-mono text-sm placeholder-gray-400/50 rounded-sm !shadow-none',
                      autocomplete: 'new-password',
                      name: 'admin-login-password',
                    },
                    showicon: {
                      class:
                        'text-gray-400 absolute right-3 cursor-pointer hover:text-brand-orange z-[3] text-sm mt-0.5',
                    },
                    hideicon: {
                      class:
                        'text-gray-400 absolute right-3 cursor-pointer hover:text-brand-orange z-[3] text-sm mt-0.5',
            ...
            <div class="pt-4">
              ...
              >
                ...
                <span
                  class="absolute top-0 left-0 w-full h-[2px] bg-white opacity-50 -translate-x-full group-hover:animate-[scanline_1s_ease-in-out_infinite]"
                ></span>

                <span v-if="!isLoading">发起连接</span>
                <span v-else class="flex flex-col items-center justify-center"> 执行中... </span>
                <i
                  v-if="!isLoading"
                  class="pi pt-0.5"
                  :class="'pi-arrow-right group-hover:translate-x-1 transition-transform'"
                ></i>
